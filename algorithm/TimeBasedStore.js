...
class TimeBasedStore2 {
  ...
  // دریافت داده بر اساس key و timestamp دقیقا برابر
  get(key, timestamp) {
    if (!this.store.has(key)) {
      return "value not found";
    }
    ...
    // جستجو برای پیدا کردن مقدار با timestamp دقیقا برابر
...
// مثال استفاده:
const store2 = new TimeBasedStore2();
store.set("key1", "test1", 1);
store.set("key1", "test2", 2);
store.set("key1", "test3", 5);
